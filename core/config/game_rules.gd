...
## Сентябрь. Перенос начала на лето — решение владельца, и оно верное: открывать
## игру второй по тяжести порой года незачем. Но расписания НПС, экономика лавок
## и фикстуры сохранений завязаны на сентябрь, и перенос роняет шесть наборов
## проверок. Он вернётся отдельным заходом, вместе с ними.
##
## На смерть от голода, из-за которой всё началось, он не влиял: чинил её не
## сезон, а то, что бот не ходил за едой.
