...
Цель теста — выбрать художественный язык всех городских окружений. Эти изображения остаются архивом сравнительных концептов; выбранный C2 уже перенесён в production-набор, а остальные варианты к игре не подключены.
...
C2 проверен на светлой площади, тёмном подземном переходе и изометрической игровой зоне поиска. После проверки в него переведены все семь текущих production-растров. Нормативные палитра, размер мазка, детализация, контраст, края, prompt и правила приёмки вынесены в [производственный стандарт окружений](../ENVIRONMENT_STYLE.md).

Targeted-edit prompt, которым был получен C2 из варианта C:
